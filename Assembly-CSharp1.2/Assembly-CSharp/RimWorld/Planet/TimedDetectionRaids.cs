...
namespace RimWorld.Planet
{
    ...
    public class TimedDetectionRaids : WorldObjectComp
    {
        ...
        {
            get { return this.ticksLeftToSendRaid >= 0 && this.ticksLeftTillNotifyPlayer == 0; }
        }
        ...
        {
            get { return this.parent.Faction ?? Faction.OfMechanoids; }
        }
        ...
        /// <param name="notifyTicks"></param>
        public void StartDetectionCountdown(int ticks, int notifyTicks = -1)
        {
            this.ticksLeftToSendRaid = ticks;
            this.ticksLeftTillNotifyPlayer = ((notifyTicks == -1)
                ? Mathf.Min((int) (60000f * Rand.Range(0.8f, 1.2f)), ticks / 2)
                : notifyTicks);
        }
        ...
        {
            this.ticksLeftTillNotifyPlayer = (ticksLeftToSendRaid = -1);
        }
        ...
        public override string CompInspectStringExtra()
        {
            ...
            if (Prefs.DevMode)
            {
                ...
                {
                    text = text + "[DEV]: Time left to send raid: " +
                           this.ticksLeftToSendRaid.ToStringTicksToPeriod(true, false, true, true) + "\n";
                }

                if (this.ticksLeftTillNotifyPlayer != -1)
                {
                    text = text + "[DEV]: Time left till notify player about incoming raid: " +
                           this.ticksLeftTillNotifyPlayer.ToStringTicksToPeriod(true, false, true, true) + "\n";
                }
            }
        ...
        private void NotifyPlayer()
        {
            Find.LetterStack.ReceiveLetter("LetterLabelSiteCountdownStarted".Translate(),
                "LetterTextSiteCountdownStarted".Translate(this.ticksLeftToSendRaid.ToStringTicksToDays("F1"),
                    this.RaidFaction.def.pawnsPlural, this.RaidFaction), LetterDefOf.ThreatBig, this.parent, null, null,
                null, null);
        }
        ...
        public override IEnumerable<Gizmo> GetGizmos()
        {
            if (Prefs.DevMode)
            {
                yield return new Command_Action
                {
                    ...
                    action = delegate() { this.ticksLeftToSendRaid = 2500; }
                };
                yield return new Command_Action
                {
                    defaultLabel = "Dev: Set notify raid timer to 1 hour",
                    action = delegate() { this.ticksLeftTillNotifyPlayer = 2500; }
                };
            }
