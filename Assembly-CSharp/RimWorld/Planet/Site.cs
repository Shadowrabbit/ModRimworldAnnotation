...
namespace RimWorld.Planet
{
    ...
    public class Site : MapParent
    {
        ...
        public string ApproachOrderString
        {
            get
            {
                return this.MainSitePartDef.approachOrderString.NullOrEmpty()
                    ? "ApproachSite".Translate(this.Label)
                    : this.MainSitePartDef.approachOrderString.Formatted(this.Label);
            }
        }
        ...
        public string ApproachingReportString
        {
            get
            {
                return this.MainSitePartDef.approachingReportString.NullOrEmpty()
                    ? "ApproachingSite".Translate(this.Label)
                    : this.MainSitePartDef.approachingReportString.Formatted(this.Label);
            }
        }
            ...
            get
            {
                ...
                for (int i = 0; i < questsListForReading.Count; i++)
                {
                    ...
                    if (quest.State == QuestState.Ongoing)
                    {
                        ...
                        {
                            QuestPart_WorldObjectTimeout questPart_WorldObjectTimeout =
                                quest.PartsListForReading[j] as QuestPart_WorldObjectTimeout;
                            if (questPart_WorldObjectTimeout != null && questPart_WorldObjectTimeout.State == QuestPartState.Enabled &&
                                questPart_WorldObjectTimeout.worldObject == this)
                            {
        ...
        public override bool ShouldRemoveMapNow(out bool alsoRemoveWorldObject)
        {
            ...
            {
                alsoRemoveWorldObject = !this.parts.Any((SitePart x) =>
            ...
            CompLaunchable representative)
        {
            ...
            IEnumerator<FloatMenuOption> enumerator = null;
            foreach (FloatMenuOption floatMenuOption2 in TransportPodsArrivalAction_VisitSite.GetFloatMenuOptions(representative, pods,
                this))
            {
                yield return floatMenuOption2;
            }
        ...
        private void CheckRecordAssaultSuccessfulTale()
        {
            if (this.anyEnemiesInitially && !this.caravanAssaultSuccessfulTaleRecorded &&
                !GenHostility.AnyHostileActiveThreatToPlayer(base.Map, false))
            {
        ...
        public override string GetInspectString()
        {
            ...
            for (int i = 0; i < this.parts.Count; i++)
            {
                ...
                {
                    if (this.MainSitePart == this.parts[i] && !this.parts[i].def.mainPartAllThreatsLabel.NullOrEmpty() &&
                        this.ActualThreatPoints > 0f)
                    {
