        ...
        public override void LordToilTick()
        {
            if (cover && Find.TickManager.TicksGame % 181 == 0)
            {
                ...
                for (int i = 0; i < this.lord.ownedPawns.Count; i++)
                {
                    ...
                    {
                        Thing thing = null;
                        //找到了目标 并且没有被集群内其他人保留 并且不是很危险
                        if (this.TryFindGoodOpportunisticTaskTarget(pawn, out thing, list) &&
                            !base.Map.reservationManager.IsReservedByAnyoneOf(thing, this.lord.faction) && !GenAI.InDangerousCombat(pawn))
                        {
                            //更换职责
                            pawn.mindState.duty = new PawnDuty(this.DutyDef);
                            pawn.jobs.EndCurrentJob(JobCondition.InterruptForced, true, true);
