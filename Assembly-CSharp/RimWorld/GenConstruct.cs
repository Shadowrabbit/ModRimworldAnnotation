...
namespace RimWorld
{
    ...
    public static class GenConstruct
    {
        ...
        public static Blueprint_Build PlaceBlueprintForBuild(BuildableDef sourceDef, IntVec3 center, Map map,
        ...
        public static Blueprint_Install PlaceBlueprintForInstall(MinifiedThing itemToInstall, IntVec3 center, Map map,
            Rot4 rotation, Faction faction)
        {
            Blueprint_Install blueprint_Install =
                (Blueprint_Install) ThingMaker.MakeThing(itemToInstall.InnerThing.def.installBlueprintDef, null);
        ...
        public static Blueprint_Install PlaceBlueprintForReinstall(Building buildingToReinstall, IntVec3 center,
            Map map, Rot4 rotation, Faction faction)
        {
            Blueprint_Install blueprint_Install =
                (Blueprint_Install) ThingMaker.MakeThing(buildingToReinstall.def.installBlueprintDef, null);
        ...
        public static bool CanBuildOnTerrain(BuildableDef entDef, IntVec3 c, Map map, Rot4 rot,
            Thing thingToIgnore = null, ThingDef stuffDef = null)
        {
        ...
        public static bool CanConstruct(Thing t, Pawn p, bool checkSkills = true, bool forced = false)
        {
            ...
            if (!p.CanReserveAndReach(t, PathEndMode.Touch, forced ? Danger.Deadly : p.NormalMaxDanger(), 1, -1, null,
                forced))
            {
                return false;
            }
        ...
        public static AcceptanceReport CanPlaceBlueprintAt(BuildableDef entDef, IntVec3 center, Rot4 rot, Map map,
            bool godMode = false, Thing thingToIgnore = null, Thing thing = null, ThingDef stuffDef = null)
        {
            //计算占据矩形
                    ...
                    if (list[j] != thingToIgnore)
                    {
                        ...
                        if (list[j].def.passability == Traversability.Impassable || list[j].def == thingDef)
                        {
                            return new AcceptanceReport("InteractionSpotBlocked"
                                .Translate(list[j].LabelNoCount, list[j]).CapitalizeFirst());
                        }

                        BuildableDef entityDefToBuild = list[j].def.entityDefToBuild;
                        if (entityDefToBuild != null && (entityDefToBuild.passability == Traversability.Impassable ||
                                                         entityDefToBuild == thingDef))
                        {
                ...
                if (c3.InBounds(map))
                {
                    ...
                    for (int k = 0; k < thingList.Count; k++)
                    {
                        ...
                        if (thing3 != thingToIgnore)
                        {
                            ...
                            //蓝图不同 并且遮挡交互
                            if (thingDef3.hasInteractionCell &&
                                (entDef.passability == Traversability.Impassable || entDef == thingDef3) &&
                                cellRect.Contains(ThingUtility.InteractionCellWhenAt(thingDef3, thing3.Position,
                                    thing3.Rotation, thing3.Map)))
                            {
                                return new AcceptanceReport("WouldBlockInteractionSpot"
                                    .Translate(entDef.label, thingDef3.label).CapitalizeFirst());
                            }
                        }
            ...
            if (GenConstruct.CanBuildOnTerrain(entDef, center, map, rot, thingToIgnore, stuffDef))
            {
                if (ModsConfig.RoyaltyActive)
                {
                    ...
                    for (int l = 0; l < list2.Count; l++)
                    {
                        ...
                        if (!monumentMarker.complete &&
                            !monumentMarker.AllowsPlacingBlueprint(entDef, center, rot, stuffDef))
                        {
                ...
                if (entDef.PlaceWorkers != null)
                {
                    ...
                    {
                        AcceptanceReport result = entDef.PlaceWorkers[n]
                            .AllowsPlacing(entDef, center, rot, map, thingToIgnore, thing);
                        if (!result.Accepted)
                        {
                            return result;
                        }
            ...
            {
                return new AcceptanceReport("TerrainCannotSupport_TerrainAffordanceFromStuff"
                    .Translate(entDef, entDef.GetTerrainAffordanceNeed(stuffDef), stuffDef).CapitalizeFirst());
            }

            return new AcceptanceReport("TerrainCannotSupport_TerrainAffordance"
                .Translate(entDef, entDef.GetTerrainAffordanceNeed(stuffDef)).CapitalizeFirst());
        }
        ...
        public static bool CanPlaceBlueprintOver(BuildableDef newDef, ThingDef oldDef)
        {
            ...
            if (terrainDef != null)
            {
                ...
                else if (oldDef.category == ThingCategory.Building &&
                         !terrainDef.affordances.Contains(oldDef.terrainAffordanceNeeded))
                {
                    return false;
                }
            }
            ...
            if (oldDef.category == ThingCategory.Plant && oldDef.passability == Traversability.Impassable &&
                thingDef != null && thingDef.category == ThingCategory.Building &&
                !thingDef.building.canPlaceOverImpassablePlant)
            {
                return false;
            }
            ...
            if (oldDef.category == ThingCategory.Building || oldDef.IsBlueprint || oldDef.IsFrame)
            {
                if (thingDef != null)
                {
                    ...
                    {
                        return (oldDef.building == null || oldDef.building.canBuildNonEdificesUnder) &&
                               (!thingDef.EverTransmitsPower || !oldDef.EverTransmitsPower);
                    }

                    if (thingDef.IsEdifice() && oldDef != null && oldDef.category == ThingCategory.Building &&
                        !oldDef.IsEdifice())
                    {
                        return thingDef.building == null || thingDef.building.canBuildNonEdificesUnder;
                    }

                    if (thingDef2 != null && (thingDef2 == ThingDefOf.Wall || thingDef2.IsSmoothed) &&
                        thingDef.building != null && thingDef.building.canPlaceOverWall)
                    {
                        return true;
                    }
                ...
                return (newDef is TerrainDef && buildableDef is ThingDef &&
                        ((ThingDef) buildableDef).CoexistsWithFloors) ||
                       (buildableDef is TerrainDef && !(newDef is TerrainDef));
            }
        ...
        public static Thing FirstBlockingThing(Thing constructible, Pawn pawnToIgnore)
        {
            ...
            foreach (IntVec3 c in constructible.OccupiedRect())
            {
                ...
                {
                    Thing thing2 = thingList[i];
                    if (GenConstruct.BlocksConstruction(constructible, thing2) && thing2 != pawnToIgnore &&
                        thing2 != thing)
                    {
                        return thing2;
                    }
        ...
        public static Job HandleBlockingThingJob(Thing constructible, Pawn worker, bool forced = false)
        {
            ...
            {
                if (worker.CanReserveAndReach(thing, PathEndMode.ClosestTouch, worker.NormalMaxDanger(), 1, -1, null,
                    forced))
                {
            ...
            else if (thing.def.category == ThingCategory.Building)
            {
                if (((Building) thing).DeconstructibleBy(worker.Faction))
                {
                    ...
                    if (worker.CanReserveAndReach(thing, PathEndMode.Touch, worker.NormalMaxDanger(), 1, -1, null,
                        forced))
                    {
        ...
        public static bool BlocksConstruction(Thing constructible, Thing t)
        {
            ...
            {
                return t.def.plant.harvestWork > ThingDefOf.Plant_Dandelion.plant.harvestWork &&
                       (!(thingDef.entityDefToBuild is TerrainDef) || !t.Spawned ||
                        !(t.Position.GetEdifice(t.Map) is IPlantToGrowSettable));
            }
            ...
            if (thingDef2 != null)
            {
                if (thingDef2.EverTransmitsPower && t.def == ThingDefOf.PowerConduit &&
                    thingDef2 != ThingDefOf.PowerConduit)
                {
                    return false;
                }
            ...
            return (t.def.IsEdifice() && thingDef2.IsEdifice()) ||
                   (t.def.category == ThingCategory.Pawn || (t.def.category == ThingCategory.Item &&
                                                             thingDef.entityDefToBuild.passability ==
                                                             Traversability.Impassable)) ||
                   (t.def.Fillage >= FillCategory.Partial && (t.def.IsEdifice() ||
                                                              (t.def.entityDefToBuild != null &&
                                                               t.def.entityDefToBuild.IsEdifice())));
        }
