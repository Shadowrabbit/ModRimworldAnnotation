...
namespace RimWorld
{
    ...
    public class RaidStrategyWorker_ImmediateAttackSappers : RaidStrategyWorker
    {
        ...
        {
            return this.PawnGenOptionsWithSappers(parms.faction, groupKind).Any<PawnGroupMaker>() &&
        ...
        private float CheapestSapperCost(Faction faction, PawnGroupKindDef groupKind)
        {
            ...
            {
                Log.Error(
                    string.Concat("Tried to get MinimumPoints for ", GetType().ToString(), " for faction ",
                        faction.ToString(), " but the faction has no groups with sappers. groupKind=", groupKind));
                return 99999f;
            }
        ...
        {
            return (otherPawns.Count != 0 || SappersUtility.IsGoodSapper(p) || SappersUtility.IsGoodBackupSapper(p)) &&
                   (!p.kindDef.canBeSapper || !SappersUtility.HasBuildingDestroyerWeapon(p) ||
                    SappersUtility.IsGoodSapper(p));
        }
