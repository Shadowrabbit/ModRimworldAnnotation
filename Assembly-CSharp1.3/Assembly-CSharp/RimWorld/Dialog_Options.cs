...
namespace RimWorld
{
	...
	public class Dialog_Options : Window
	{
		...
		// (get) Token: 0x060074BA RID: 29882 RVA: 0x0027A8D0 File Offset: 0x00278AD0
		...
		// Token: 0x060074BB RID: 29883 RVA: 0x0027A8E1 File Offset: 0x00278AE1
		...
		// Token: 0x060074BC RID: 29884 RVA: 0x0027A905 File Offset: 0x00278B05
		public override void PostOpen()
		{
			base.PostOpen();
			this.simulateNotOwningRoyaltyWhenOpened = Prefs.SimulateNotOwningRoyalty;
			this.simulateNotOwningIdeologyWhenOpened = Prefs.SimulateNotOwningIdology;
		}

		// Token: 0x060074BD RID: 29885 RVA: 0x0027A924 File Offset: 0x00278B24
		public override void DoWindowContents(Rect inRect)
		{
			...
			if (listing_Standard.ButtonTextLabeled("Resolution".Translate(), Dialog_Options.ResToString(Screen.width, Screen.height)))
			{
				...
				{
					list.Add(new FloatMenuOption("NoneBrackets".Translate(), null, MenuOptionPriority.Default, null, null, 0f, null, null, true, 0));
				}
			...
			if ((from e in ModLister.AllExpansions
			where !e.isCore && e.Status != ExpansionStatus.NotInstalled
			select e).Any<ExpansionDef>() && listing_Standard.ButtonTextLabeled("SetBackgroundImage".Translate(), Prefs.BackgroundImageExpansion.LabelCap))
			{
				List<FloatMenuOption> list4 = new List<FloatMenuOption>();
				foreach (ExpansionDef expansionDef in ModLister.AllExpansions)
				{
					if (expansionDef.Status != ExpansionStatus.NotInstalled)
					{
						ExpansionDef localExpansion = expansionDef;
						list4.Add(new FloatMenuOption(expansionDef.label, delegate()
						{
							Prefs.BackgroundImageExpansion = localExpansion;
						}, expansionDef.Icon, Color.white, MenuOptionPriority.Default, null, null, 0f, null, null, true, 0));
					}
				}
			...
			if (listing_Standard.ButtonText("ChooseLanguage".Translate(), null))
			{
				...
				else
				{
					...
					foreach (LoadedLanguage localLang2 in LanguageDatabase.AllLoadedLanguages)
					{
						...
						list5.Add(new FloatMenuOption(localLang.DisplayName, delegate()
						{
			...
			if (listing_Standard.ButtonTextLabeled("AutomaticPauseModeSetting".Translate(), Prefs.AutomaticPauseMode.ToStringHuman()))
			{
				...
				foreach (object obj2 in Enum.GetValues(typeof(AutomaticPauseMode)))
				{
					...
					list6.Add(new FloatMenuOption(localPmode.ToStringHuman(), delegate()
					{
			...
			if (listing_Standard.ButtonTextLabeled("TemperatureMode".Translate(), Prefs.TemperatureMode.ToStringHuman()))
			{
				...
				foreach (object obj3 in Enum.GetValues(typeof(TemperatureDisplayMode)))
				{
					...
					list7.Add(new FloatMenuOption(localTmode.ToStringHuman(), delegate()
					{
			...
			if (listing_Standard.ButtonTextLabeled("AutosaveInterval".Translate(), autosaveIntervalDays + " " + ((autosaveIntervalDays == 1f) ? text2 : text)))
			{
				...
				if (Prefs.DevMode)
				{
					list8.Add(new FloatMenuOption("0.125 " + text + "(debug)", delegate()
					{
						Prefs.AutosaveIntervalDays = 0.125f;
					}, MenuOptionPriority.Default, null, null, 0f, null, null, true, 0));
					list8.Add(new FloatMenuOption("0.25 " + text + "(debug)", delegate()
					{
						Prefs.AutosaveIntervalDays = 0.25f;
					}, MenuOptionPriority.Default, null, null, 0f, null, null, true, 0));
				}
				list8.Add(new FloatMenuOption(("0.5 " + text) ?? "", delegate()
				{
					Prefs.AutosaveIntervalDays = 0.5f;
				}, MenuOptionPriority.Default, null, null, 0f, null, null, true, 0));
				list8.Add(new FloatMenuOption(1.ToString() + " " + text2, delegate()
				{
					Prefs.AutosaveIntervalDays = 1f;
				}, MenuOptionPriority.Default, null, null, 0f, null, null, true, 0));
				list8.Add(new FloatMenuOption(3.ToString() + " " + text, delegate()
				{
					Prefs.AutosaveIntervalDays = 3f;
				}, MenuOptionPriority.Default, null, null, 0f, null, null, true, 0));
				list8.Add(new FloatMenuOption(7.ToString() + " " + text, delegate()
				{
					Prefs.AutosaveIntervalDays = 7f;
				}, MenuOptionPriority.Default, null, null, 0f, null, null, true, 0));
				list8.Add(new FloatMenuOption(14.ToString() + " " + text, delegate()
				{
			...
			if (Prefs.DevMode)
			{
				...
				if (Current.ProgramState != ProgramState.Playing)
				{
					...
					bool simulateNotOwningIdology = Prefs.SimulateNotOwningIdology;
					listing_Standard.CheckboxLabeled("SimulateNotOwningIdeology".Translate(), ref simulateNotOwningIdology, null);
					Prefs.SimulateNotOwningIdology = simulateNotOwningIdology;
				}
			}
			listing_Standard.End();
		}

		// Token: 0x060074BE RID: 29886 RVA: 0x0027BBBC File Offset: 0x00279DBC
		public override void PreClose()
		{
			...
			if (Prefs.SimulateNotOwningIdology && !this.simulateNotOwningIdeologyWhenOpened && ModsConfig.IdeologyActive)
			{
		...
		// Token: 0x060074BF RID: 29887 RVA: 0x0027BC20 File Offset: 0x00279E20
		...
		// Token: 0x060074C0 RID: 29888 RVA: 0x0027BC80 File Offset: 0x00279E80
